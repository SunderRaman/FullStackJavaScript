            ...
            if (err) {
                ...
                sharp('./server/storage/' + ctx.req.params.container + '/' + fileInfo.name)
                     ...
                     .toFile('./server/storage/' + ctx.req.params.container + '/100-' + fileInfo.name, (err) => {
                        if(!err) {
                            /* + '/download/'  /download/  These were earlier in url and thumbnail */
                            PostImage.create({
                                url: CONTAINER_URL + fileInfo.container + fileInfo.name,
                                thumbnail: CONTAINER_URL + fileInfo.container + '100-' + fileInfo.name,
                                created_at:new Date(),
                                postId: post_id,
                                userId: user_id,
                            }, (err2, image)=> {
